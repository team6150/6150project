...
var contextAwareTemplateParser_1 = require("./templates/contextAwareTemplateParser");
...
var Ng2Walker = (function (_super) {
    ...
    Ng2Walker.prototype.visitNg2Component = function (metadata) {
        ...
        if (template && template.template) {
            try {
                var templateAst = void 0;
                if (!this._config.languageService) {
                    templateAst = templateParser_1.parseTemplate(template.template.code, config_1.Config.predefinedDirectives);
                }
                else {
                    var fileName = this.getSourceFile().fileName;
                    var program = this._config.languageService.getProgram();
                    templateAst = contextAwareTemplateParser_1.parseTemplate(metadata.controller, fileName, program);
                }
    ...
    Ng2Walker.prototype.visitNg2TemplateHelper = function (roots, context, baseStart) {
        ...
        if (!roots || !roots.length) {
            ...
            var visitor = new this._config.templateVisitorCtrl(sourceFile, this._originalOptions, context, baseStart, this._config.expressionVisitorCtrl, this._config.languageService);
